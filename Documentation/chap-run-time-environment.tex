...
\texttt{undefined-function} error.  If \texttt{function-name} has a
definition as a function, then the \texttt{car} slot of the \texttt{cons}
cell contains the function object.  If \texttt{function-name} does not
...
return \texttt{nil} if no function named \textit{function-name} exists
in \textit{environment}.  Similarly, Clostrum does not provide any
methods specialized to \texttt{evaluation-environment}, so client code
must provide a method specialized to its specific \texttt{client}
class, and to (a subclass of) \texttt{evaluation-environment}.

Normally, this generic function will be called with an instance of
\texttt{compilation-environment} rather than
\texttt{run-time-environment}.  Clostrum provides methods on
\texttt{(setf function-description)} and \texttt{function-description}
...
typically an instance of \texttt{evaluation-environment}.
