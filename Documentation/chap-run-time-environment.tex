...
The value of \textit{new-type} may be \texttt{nil}.  Then any existing
type information about \textit{function-name} is erased, and the
result is as if no type information has been given.  Then, if the
function is defined, subsequent calls to \texttt{function-type} will
return \texttt{t}, and if not, such calls will return \texttt{nil}.
